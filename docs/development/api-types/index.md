---
layout: page
title: Api Types
permalink: /development/api-types/
parent: Development
has_children: true
has_toc: false
---



# API Types 
Created by Benny, last modified on 2017-02-22
This page is availble if you need a closer look at each specific type.
Under Simple Type you´ll find the common types.

- [address](address)
- [basicPayment](basicpayment)
- [bookingResult](bookingresult)
- [bookPaymentResult](bookpaymentresult)
- [bookPaymentStatus](bookpaymentstatus)
- [cardData](carddata)
- [countryCode](countrycode)
- [customer](customer)
- [customerCard](customercard)
- [customerIdentification](customeridentification)
- [customerIdentificationResponse](customeridentificationresponse)
- [customerType](customertype)
- [digestAlgorithm](digestalgorithm)
- [digestConfiguration](digestconfiguration)
- [ECommerceError](ecommerceerror)
- [formElement](formelement)
- [fraudControlStatus](fraudcontrolstatus)
- [invoiceData](invoicedata)
- [invoiceDeliveryType](invoicedeliverytype)
- [limit](limit)
- [limitApplicationFormAsCompiledForm](limitapplicationformascompiledform)
- [limitApplicationFormAsObjectGraph](limitapplicationformasobjectgraph)
- [limitDecision](limitdecision)
- [mapEntry](mapentry)
- [option](option)
- [payment](payment)
- [paymentData](paymentdata)
- [paymentDiff](paymentdiff)
- [paymentDiffType](paymentdifftype)
- [paymentMethod](paymentmethod)
- [paymentMethodType](paymentmethodtype)
- [paymentSession](paymentsession)
- [paymentSpec](paymentspec)
- [paymentStatus](paymentstatus)
- [pdf](pdf)
- [searchCriteria](searchcriteria)
- [signing object](signing-object)
- [Simple Types...](simple-types...)
- [sortAlternative](sortalternative)
- [sortOrder](sortorder)
- [specLine](specline)
- [webLink](weblink)
- [withMetaData](withmetadata)

